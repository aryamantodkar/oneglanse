...
export default function FirstWorkspaceOnboardingPage() {
  ...
  const progressPercent = useMemo(() => {
    ...
    return Math.max(5, Math.min(100, Math.round((total / states.length) * 100)));
  }, [jobStatusQuery.data?.response]);
  ...
  const analysisCount = useMemo(() => {
    ...
    if (typeof data === "object" && "records" in data && Array.isArray((data as any).records)) {
  ...
  const appendPrompt = (prompt: string) => {
    ...
    if (current.split("\n").some((line) => line.trim() === prompt.trim())) return;
        ...
        <Card className="overflow-hidden border-gray-200/80 shadow-xl dark:border-gray-800">
          <CardHeader className="border-b border-gray-100 bg-white/90 pb-5 dark:border-gray-800 dark:bg-gray-900/50">
            ...
            <CardDescription className="max-w-3xl text-sm">
              Add prompts that matter for buyers in your category. We will run and analyze them once now, then move you to your dashboard as soon as first insights are ready.
            </CardDescription>
            <p className="text-xs text-muted-foreground">
              Brand: <span className="font-medium text-gray-900 dark:text-gray-100">{brandName}</span>
            ...
            {!started ? (
              <>
                <div className="space-y-2">
                  ...
                  />
                  <p className="text-xs text-muted-foreground">
                    Add one prompt per line. We recommend starting with 6-10 prompts.
                  </p>
                </div>
              ...
              <div className="space-y-6">
                <div className="rounded-xl border border-gray-200 bg-gray-50/70 p-4 dark:border-gray-800 dark:bg-gray-900/50">
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">
                    We will redirect you to the dashboard as soon as some data is available.
                  </p>
                </div>
