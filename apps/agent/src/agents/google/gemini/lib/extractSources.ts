...
export async function extractSourcesFromGoogle(page: Page): Promise<Source[]> {
	const sources = await page.evaluate(() => {
		...
		const referencesText =
			parts && parts.length > 1 && typeof parts[parts.length - 1] === "string"
				? parts[parts.length - 1]
				: "";
		const citationLines = (referencesText ?? "")
			.split("\n")
		...
		for (const link of allLinks) {
			...
			const citationLine = citationLines.find((line) =>
				line.includes(linkText),
			);
