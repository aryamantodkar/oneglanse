...
export async function askPrompt(
	...
	const checkSubmissionSuccess = async (): Promise<boolean> => {
		...
		if (provider === "perplexity" || provider === "google-ai-overview") {
			...
			if (
				urlAfter !== preSubmitUrl &&
				(provider !== "google-ai-overview" || urlAfter.includes("/search"))
			) {
		...
		const editorContent = await input
			.evaluate((el) => {
				if (el instanceof HTMLTextAreaElement || el instanceof HTMLInputElement)
					return el.value.trim();
				return (el.textContent || "").trim();
		...
		if (preSubmitContent.length > 0 && editorContent.length === 0) {
			logger.debug(
				`  ✓ Submission detected via editor clear: ${preSubmitContent.length} → 0 chars`,
			);
			return true;
		}
		...
		const hasIndicators = await page
			.locator(generationSelector)
			.first()
			.isVisible()
			.catch(() => false);
		if (hasIndicators) {
		...
		const hasStopButton = await page
