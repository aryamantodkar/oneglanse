import type { UserPrompt, WorkspaceLocation } from "@onescope/types";
import fs from "fs";
import path from "path";
import { ExternalServiceError, safeHandler } from "@onescope/errors";
import { queryClaude, queryOpenai, queryPerplexity } from "./providers/index";
        ...
        const [gptRes, claudeRes, perplexityRes] = await Promise.all([
          safeHandler(async () => {
            try {
              ...
              throw new ExternalServiceError("OpenAI", "OpenAI request failed.", 502, { prompt }, err);
            }
          }),
          safeHandler(async () => {
            try{
              ...
              throw new ExternalServiceError("Anthropic", "Anthropic request failed.", 502, { prompt }, err);
            }
          }),
          safeHandler(async () => {
            try{
