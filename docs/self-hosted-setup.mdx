...
### Recommended ThorData configuration

Before generating the ThorData endpoint, whitelist your VPS public IP in the ThorData dashboard. That is the recommended setup for OneGlanse because it reduces authentication friction and keeps the proxy flow simpler on self-hosted VPS deployments.

When you generate the endpoint, use these settings:

- Authentication method: `Whitelisted IPs`
- Endpoint type: `API Link`
- Session setting: `Sticky session`
- Country: choose a valid country where your target providers are available and where your audience is actually located

Use the generated API link as `THORDATA_PROXY_API_URL`. Keep sticky sessions enabled for prompt runs so the provider flow stays on a stable residential IP long enough for sign-in and collection to complete reliably.
